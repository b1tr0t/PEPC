...
1.  **Accessibility**: Permission UI for a capability is triggered either
    through the direct use of the capability, or via `permission.request`.
    Typically, this involves JavaScript, which in turn invokes permission UI.
    This presents an issue for both screen readers and magnification users.
...
validation. Therefore a `:invalid` CSS pseudo-class is also proposed.
